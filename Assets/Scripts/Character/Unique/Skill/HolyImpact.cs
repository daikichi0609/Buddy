...
public class HolyImpact : Skill
{
    protected override string Name => "ホーリーインパクト";
    ...
    private static readonly string HOLY_IMPACT = "HolyImpact";
    ...
    protected override async Task SkillEffect(SkillContext ctx)
    {
        ...
        if (ctx.SoundHolder.TryGetSound(HOLY_IMPACT, out var sound) == true)
            sound.Play();

        IDisposable disposable = null;
        if (ctx.EffectHolder.TryGetEffect(HOLY_IMPACT, out var effect) == true)
